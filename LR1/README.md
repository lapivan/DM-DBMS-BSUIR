...
Базовые данные для входа в систему и общая персональная информация.
...
| PasswordHash | VARCHAR | Not Null | Хэш пароля пользователя |
...
| IsActive | BOOL | Default True | Флаг активности аккаунта (QueryFilter) |
...
| IsActive | BOOL | Default True | Флаг активности пациента (QueryFilter) |
...
| IsActive | BOOL | Default True | Флаг активности врача (QueryFilter) |
...
| GapInMonths | SMALLINT | Not Null | Перерыв в стаже в месяцах |
| IsActive | BOOL | Default True | Флаг активности администратора (QueryFilter) |
...
Медицинское заключение по итогам приема.
...
Файлы анализов или внешние справки к результату.

| Поле | Тип | Ограничения | Описание |
| --- | --- | --- | --- |
| Id | UUID | Primary Key | Идентификатор документа |
| ResultId | UUID | Foreign Key, Not Null | Ссылка на результат обследования |
| Url | VARCHAR | Not Null | Путь к файлу (например, PDF) |
...
| Action | VARCHAR | Not Null | Тип операции (например, CREATE_APPOINTMENT) |
| EntityName | VARCHAR | Null | Затронутая таблица/сущность |
...
* **Один-ко-Многим (1:M):** `Office` ↔ `Doctor`, `Office` ↔ `Administrator`, `Specialization` ↔ `Doctor`, `Specialization` ↔ `Service`, `ServiceCategory` ↔ `Service`, `Result` ↔ `Document`, `Account` ↔ `AuditLog`.
